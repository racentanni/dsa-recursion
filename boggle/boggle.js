...
function findFrom(board, word, y, x, seen) {
  /** Can word be found in board, starting at x and y? */

  //Base case: this is not the letter we're looking for
  if (board[y][x] !=word[0]) return false;

//Base case: we uded this letter before in the current path
...
//Can't find the next letter, so this patch is dead
...
function find(board, word) {
  //Can word be found on board?

  for (let y = 0; y < 5; y++)
    for (let x = 0; x < 5; x++)
      if (findFrom(board, word, y, x, new Set())) return true;

  //We have tried every path from every starting square with no luck
